...
@section('content')
    <section class="pb-6">
        ...
        <p class="mt-1 text-base text-slate-400">{{ $temporada?->nombre ?? 'Sin temporada activa' }} · {{ $seccion->resumenReglas() }}</p>
        @if ($texto)
            <div class="mt-4">
    ...
    @if ($grupo === null)
        <section class="pb-8">
            <div class="rounded-2xl border border-slate-800 bg-slate-900 p-5 text-slate-400">
                Todavía no hay mangas celebradas de esta sección. En cuanto se cierre la primera, aquí saldrá el ranking.
            </div>
        </section>
    @else
        <section class="pb-8">
            <h2 class="mb-3 text-lg font-bold text-emerald-400">Clasificación general</h2>
        ...
        @if ($clasifUltima)
            <section class="pb-8">
                ...
                <p class="mb-3 text-sm text-slate-500">
                    ...
                    · <a href="{{ route('club.manga', ['club' => $club->slug, 'manga' => $ultimaManga->id]) }}" class="text-emerald-400 hover:underline">Ver y compartir</a>
                </p>
