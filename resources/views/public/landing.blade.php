...
@section('content')
    ...
    <section class="py-10">
        <div class="mx-auto max-w-2xl rounded-3xl border border-emerald-200 bg-emerald-50 p-8 text-center">
    ...
    <section class="py-10">
        ...
        <div class="mx-auto mt-8 max-w-3xl divide-y divide-slate-200 rounded-2xl border border-slate-200 bg-white">
            <details class="group p-5">
                ...
                <p class="mt-2 text-sm text-slate-500">Cada sección se configura con sus reglas: por peso, medida o piezas; descartes; puntos por asistencia; qué decide un empate. Y la app te lo explica en una frase para que compruebes que es lo vuestro. Si falta algo que un club real necesite, lo añadimos.</p>
            </details>
            <details class="group p-5">
                <summary class="cursor-pointer list-none font-semibold">¿Los socios necesitan cuenta?
                    <span class="float-right text-slate-500 group-open:rotate-45">+</span></summary>
                <p class="mt-2 text-sm text-slate-500">Para ver las clasificaciones que compartes por WhatsApp, no: son enlaces públicos. Para decir «asistiré» y ver su puesto resaltado, sí; la crean con un toque desde el enlace que les mandas.</p>
            </details>
            <details class="group p-5">
                <summary class="cursor-pointer list-none font-semibold">¿Y si nos apañamos con Excel?
                    <span class="float-right text-slate-500 group-open:rotate-45">+</span></summary>
                <p class="mt-2 text-sm text-slate-500">Seguid con él si os funciona. Plica es para el club que cada domingo pierde una hora sumando, discute un descarte o tiene que reenviar el ranking cuatro veces. Pruébalo con una manga real: si no ahorra tiempo, no hay nada que pagar.</p>
            </details>
            <details class="group p-5">
            ...
            <details class="group p-5">
                <summary class="cursor-pointer list-none font-semibold">¿Cuánto tarda el alta?
                    <span class="float-right text-slate-500 group-open:rotate-45">+</span></summary>
                <p class="mt-2 text-sm text-slate-500">Normalmente el mismo día. Nos mandas el nombre del club y el email del que será administrador, y te llega el acceso. El resto lo resolvemos por WhatsApp: secciones, socios y la primera manga, sin reuniones ni tutoriales.</p>
            </details>
        </div>
    </section>
    ...
    <section id="solicitar" class="mx-auto max-w-lg py-10">
        <div class="rounded-2xl border border-slate-200 bg-white p-6">
            ...
            <p class="mt-1 text-sm text-slate-500">
                Sin registros ni configuraciones: cuéntanos tu club y <span class="text-slate-800">te lo dejamos montado nosotros</span>.
                Recibirás tu acceso de administrador listo para entrar, normalmente el mismo día.
            </p>

            @if (session('solicitud_ok'))
                <div class="mt-4 rounded-lg border border-emerald-300 bg-emerald-50 p-3 text-sm text-emerald-700">
            ...
            @else
                <form method="POST" action="{{ route('solicitud.store') }}" class="mt-4 space-y-3">
                    ...
                    <div>
                        <label class="mb-1 block text-sm text-slate-700" for="club_nombre">Nombre del club</label>
                        <input id="club_nombre" name="club_nombre" required maxlength="120" value="{{ old('club_nombre') }}"
                               class="w-full rounded-lg border border-slate-300 bg-slate-50 px-3 py-2.5 text-base focus:border-emerald-500 focus:outline-none">
                        @error('club_nombre') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="mb-1 block text-sm text-slate-700" for="email">Email del que será administrador</label>
                        <input id="email" name="email" type="email" required maxlength="120" value="{{ old('email') }}"
                               class="w-full rounded-lg border border-slate-300 bg-slate-50 px-3 py-2.5 text-base focus:border-emerald-500 focus:outline-none">
                        @error('email') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="mb-1 block text-sm text-slate-700" for="mensaje">Cuéntanos algo del club <span class="text-slate-500">(opcional)</span></label>
