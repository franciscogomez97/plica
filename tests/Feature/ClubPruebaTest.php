...
class ClubPruebaTest extends TestCase
{
    ...
    public function test_el_club_de_pruebas_esta_completo(): void
    {
        ...
        $this->assertSame(57, $club->seccions()->count()); // 3 de la demo + 24 de «suma lo pescado» + 30 de la federación
        ...
        // Mangas: 12 pesadas con ranking (+72 de la matriz de suma y +90 de la de federación), 2 pasadas sin pesar (pendientes) y 3 próximas con asistencias.
        $mangas = Manga::whereHas('temporada', fn ($q) => $q->where('club_id', $club->id))->get();
        $this->assertCount(17 + 72 + 90, $mangas);
        $this->assertSame(12 + 72 + 90, $mangas->where('estado', Manga::ESTADO_CELEBRADA)->count());
    ...
    public function test_volver_a_ejecutarlo_reinicia_el_club_sin_cambiar_ids_ni_duplicar_nada(): void
    {
        ...
        $this->assertSame(57, $club->seccions()->count());
        $this->assertSame(0, User::where('email', 'tester@club.com')->count());
        $this->assertSame(17 + 72 + 90, Manga::whereHas('temporada', fn ($q) => $q->where('club_id', $club->id))->count());
